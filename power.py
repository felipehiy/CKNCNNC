...
# Install dependencies: all tools automatically

def install_deps():
    slow_print("
🔄 Actualizando repositorios…")
    subprocess.run(['pkg','update','-y'], check=False)
    slow_print("🛠️ Instalando herramientas de pentest…")
    tools = [
    ...
    for pkg in tools:
        slow_print(f"Instalando {pkg}… Espere…")
        subprocess.run(['pkg','install','-y', pkg], check=False)
    slow_print("✅ Herramientas instaladas.")
    slow_print("Instalando dependencias Python…")
    subprocess.run([sys.executable, '-m', 'pip', 'install', '--upgrade',
                    'theharvester','mpremote','adafruit-ampy'], check=False)
    slow_print("Instalando WPScan…")
    # instalar gem wpscan sin sudo
    subprocess.run(['gem','install','wpscan','--user-install'], check=False)
    slow_print("✅ Dependencias PIP y gem instaladas.")
    pause()
